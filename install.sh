...
WEBROOT="/var/www/${APP}"
RAW_BASE="https://raw.githubusercontent.com/zyanv/VPN/main"
RAW_SCRIPT_URL="${RAW_BASE}/xray-vpn-manager-v5.2.sh"
...
trap 'rc=$?; printf "\n%bRalat pada baris %s (kod %s). Log pemasangan: /tmp/%s.last.log%b\n" "$C_RED" "$LINENO" "$rc" "$APP" "$C_RESET"' ERR
...
tty_read_secret(){
  ...
  local prompt="${1:-}"
  if [[ -r /dev/tty ]]; then
    IFS= read -r -s -p "$prompt" "$__var" </dev/tty || return 1
  else
    IFS= read -r -s -p "$prompt" "$__var" || return 1
  fi
  printf '\n'
}
pause(){ local _pause=""; tty_read _pause "Tekan Enter untuk kembali..." || true; }
...
add_user(){
  ...
  tty_read name "Nama user: " || return; [[ "$name" =~ ^[A-Za-z0-9._-]{2,32}$ ]] || { echo "Nama tidak sah."; return; }
  tty_read days "Tempoh hari [30]: " || return; days=${days:-30}; [[ "$days" =~ ^[0-9]+$ ]] || return
...
delete_user(){ local name tmp; list_users; tty_read name "Nama user untuk delete: " || return; tmp=$(mktemp); jq --arg n "$name" 'map(select(.name!=$n))' "$USERS" > "$tmp" && mv "$tmp" "$USERS"; reload_all; }
renew_user(){ local name days tmp; list_users; tty_read name "Nama user: " || return; tty_read days "Tambah hari [30]: " || return; days=${days:-30}; tmp=$(mktemp); jq --arg n "$name" --argjson d "$days" 'map(if .name==$n then .expiry=((if .expiry>now then .expiry else now end)+($d*86400)|floor) else . end)' "$USERS" > "$tmp" && mv "$tmp" "$USERS"; reload_all; }
...
show_user_config(){
  ...
  [[ -n "$name" ]] || { list_users; tty_read name "Nama user: " || return; }
...
vless_menu(){
  ...
  while true; do clear_screen; header "$title"; two_col "1) Add user" "5) Renew user" "2) Trial user" "6) Remove expired" "3) List user" "7) Check config user" "4) Delete user" "0) Back"; menu_prompt; tty_read c "" || return
...
add_proxy(){
  ...
  tty_read type "Jenis [socks5/http]: " || return; [[ "$type" == socks5 || "$type" == http ]] || return
  tty_read host "IP/host: " || return; [[ -n "$host" ]] || return
  tty_read port "Port: " || return; valid_port "$port" || return
  tty_read user "Username (kosong jika tiada): " || return; tty_read_secret pass "Password: " || return
  tty_read weight "Weight [1-20, default 1]: " || return; weight=${weight:-1}; [[ "$weight" =~ ^[0-9]+$ ]] && ((weight>=1&&weight<=20)) || weight=1
...
set_proxy_weight(){ local id weight tmp; list_proxies; tty_read id "ID proxy: " || return; tty_read weight "Weight [1-20]: " || return; [[ "$weight" =~ ^[0-9]+$ ]] && ((weight>=1&&weight<=20)) || { echo "Weight tidak sah."; return; }; tmp=$(mktemp); jq --arg id "$id" --argjson w "$weight" 'map(if .id==$id then .weight=$w else . end)' "$PROXIES" > "$tmp" && mv "$tmp" "$PROXIES"; reload_all; }
toggle_proxy_item(){ local id tmp; list_proxies; tty_read id "ID proxy: " || return; tmp=$(mktemp); jq --arg id "$id" 'map(if .id==$id then .enabled=(.enabled|not) else . end)' "$PROXIES" > "$tmp" && mv "$tmp" "$PROXIES"; reload_all; }
delete_proxy(){ local id tmp; list_proxies; tty_read id "ID proxy delete: " || return; tmp=$(mktemp); jq --arg id "$id" 'map(select(.id!=$id))' "$PROXIES" > "$tmp" && mv "$tmp" "$PROXIES"; reload_all; }
check_proxy(){
  local id p type host port user pass
  list_proxies; tty_read id "ID proxy check: " || return; p=$(jq -c --arg id "$id" '.[]|select(.id==$id)' "$PROXIES"); [[ -n "$p" ]] || return
...
edit_domains(){
  while true; do clear_screen; header "Domain Bypass Proxy"; nl -ba "$DOMAINS"; echo; echo "1) Add domain  2) Delete line  0) Back"; menu_prompt; tty_read c "" || return
    case "$c" in 1) tty_read d "Domain/rule (contoh domain:netflix.com): " || return; [[ -n "$d" ]] && echo "$d" >> "$DOMAINS"; reload_all;; 2) tty_read n "Nombor baris: " || return; [[ "$n" =~ ^[0-9]+$ ]] && sed -i "${n}d" "$DOMAINS"; reload_all;;0)return;;esac
  done
}
set_lb_strategy(){
  ...
  tty_read c "Pilih strategi: " || return
  case "$c" in
...
set_lb_probe(){
  local url interval
  tty_read url "Probe URL [https://www.google.com/generate_204]: " || return
  tty_read interval "Interval [30s]: " || return
...
proxy_menu(){
  while true; do clear_screen; header "SOCKS5 / HTTP Outbound"; two_col "1) Enable/disable sistem" "7) Check proxy" "2) Mode full/domain" "8) Domain bypass list" "3) Load balancing" "9) Show config" "4) Add proxy" "10) Set proxy weight" "5) Enable proxy item" "11) Health dashboard" "6) Delete proxy" "0) Back"; menu_prompt; tty_read c "" || return
    case "$c" in
...
set_domain(){ local d; tty_read d "Domain: " || return; valid_domain "$d" || { echo "Domain tidak sah."; return; }; state_set --arg d "$d" '.domain=$d'; write_nginx; systemctl reload nginx; }
set_dns(){
  echo "1) Cloudflare  2) Google  3) Quad9  4) Custom"; menu_prompt; tty_read c "" || return
  case "$c" in
    ...
    4) local a b; tty_read a "DNS 1: " || return; tty_read b "DNS 2: " || return; state_set --arg a "$a" --arg b "$b" '.dns={preset:"custom",servers:[$a,$b]}';;
  esac
  reload_all
}
toggle_ipv6(){
...
set_autoreboot(){
  ...
  if [[ "$enabled" == true ]]; then rm -f /etc/cron.d/${APP}-reboot; state_set '.autoreboot.enabled=false'; else
    tty_read time "Masa HH:MM [04:30]: " || return; time=${time:-04:30}; [[ "$time" =~ ^([01][0-9]|2[0-3]):[0-5][0-9]$ ]] || return
    h=${time%:*}; m=${time#*:}; echo "$m $h * * * root /sbin/reboot" > /etc/cron.d/${APP}-reboot
    state_set --arg t "$time" '.autoreboot={enabled:true,time:$t}'
  fi
}
choose_xray_channel(){ echo "1) Stable release  2) Latest termasuk pre-release"; menu_prompt; tty_read c "" || return; [[ "$c" == 2 ]] && state_set '.xrayChannel="latest"' || state_set '.xrayChannel="stable"'; install_xray; reload_all; }
system_menu(){
  while true; do clear_screen; header "System"; two_col "1) Set domain" "6) Restart all service" "2) Issue/Renew SSL ACME" "7) Enable/disable IPv6" "3) Set DNS" "8) Pilih/update Xray Core" "4) Set autoreboot" "9) Show logs" "5) Test config" "0) Back"; menu_prompt; tty_read c "" || return
...
install_menu_binary(){
  local src_path="" tmp=""
  if [[ -f "$0" && "$0" != /proc/* && "$(basename "$0")" != "bash" && "$(basename "$0")" != "sh" ]]; then
    src_path=$(readlink -f "$0" 2>/dev/null || printf '%s' "$0")
    if [[ "$src_path" != "$MENU_BIN" ]]; then
    ...
    curl -fL --retry 3 --connect-timeout 15 "$RAW_SCRIPT_URL" -o "$tmp"
    bash -n "$tmp"
    install -m 755 "$tmp" "$MENU_BIN"
    rm -f "$tmp"
  fi
}
...
uninstall_all(){
  tty_read x "Taip REMOVE untuk uninstall: " || return; [[ "$x" == REMOVE ]] || return
  ...
  tty_read y "Padam data user/config juga? [y/N]: " || return; [[ "$y" =~ ^[Yy]$ ]] && rm -rf "$BASE" "$WEBROOT"
...
main_menu(){
  ...
  while true; do
    ...
    menu_prompt; tty_read c "" || { echo; return; }
...
if [[ $# -eq 0 ]]; then
  ...
  case "$1" in
    install) init_state; install_all;;
    uninstall) init_state; uninstall_all;;
    menu) main_menu;;
    *) echo "Usage: $0 [install|uninstall|menu]"; exit 1;;
  esac
fi
