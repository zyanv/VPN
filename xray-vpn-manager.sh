...
build_config(){
  ...
  jq -n \
        ...
        else [] end));
    {
      ...
      inbounds:[
        ...
        {tag:"vless-xhttp",listen:"127.0.0.1",port:$xhPort,protocol:"vless",settings:{clients:clients,decryption:"none"},streamSettings:{network:"xhttp",xhttpSettings:{path:$xhPath}},sniffing:{enabled:true,destOverride:["http","tls","quic"],routeOnly:true}}
      ],
      outbounds:$outbounds,
  ...
  if ! "$XRAY_BIN" run -test -format=json -c "$tmp"; then
    echo "Konfigurasi Xray tidak sah. Fail ujian disimpan di: $tmp" >&2
    return 1
  fi
