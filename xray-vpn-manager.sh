#!/usr/bin/env bash
# ZYANV VPN Manager v4 - idempotent single-file Xray/Nginx manager
...
build_config(){
  ...
  jq -n \
        ...
        else [] end));
    {
      ...
      outbounds:$outbounds,
      routing:{domainStrategy:"IPIfNonMatch",rules:routeRules,balancers:(if $proxyEnabled and (pTags|length)>0 then [{tag:"proxy-balancer",selector:["proxy-"],fallbackTag:"direct",strategy:{type:"random"}}] else [] end)}
    }' > "$tmp"
