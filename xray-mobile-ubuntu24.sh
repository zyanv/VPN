...
"proxy":{"enabled":False,"mode":"off","selected":[],"domains":[],"rotate":False},
...
def install_xray(ver="latest"):
 data=json.loads(sh(["curl","-fsSL","https://api.github.com/repos/XTLS/Xray-core/releases/latest"],cap=True))
 tag=data["tag_name"] if ver=="latest" else (ver if ver.startswith("v") else "v"+ver)
 arch={"x86_64":"64","aarch64":"arm64-v8a"}.get(os.uname().machine)
 if not arch: raise RuntimeError("Unsupported arch")
 url=f"https://github.com/XTLS/Xray-core/releases/download/{tag}/Xray-linux-{arch}.zip"
 with tempfile.TemporaryDirectory() as td:
  z=Path(td)/"x.zip"; sh(["curl","-fL",url,"-o",str(z)]); sh(["unzip","-oq",str(z),"-d",td])
  shutil.copy2(Path(td)/"xray",XB); os.chmod(XB,0o755)
  g=Path("/usr/local/share/xray"); g.mkdir(parents=True,exist_ok=True)
  for f in ("geoip.dat","geosite.dat"):
   if (Path(td)/f).exists(): shutil.copy2(Path(td)/f,g/f)
 return tag
def service():
 Path("/etc/systemd/system/xray.service").write_text("[Unit]\nDescription=Xray\nAfter=network-online.target\n[Service]\nUser=nobody\nGroup=nogroup\nExecStart=/usr/local/bin/xray run -config /usr/local/etc/xray/config.json\nRestart=on-failure\nLimitNOFILE=1048576\n[Install]\nWantedBy=multi-user.target\n")
...
def genx(d):
 ...
 if d["proxy"]["enabled"] and tags:
  if d["proxy"]["rotate"] and len(tags)>1:
   rt["balancers"]=[{"tag":"proxy-pool","selector":tags,"strategy":{"type":"random"}}]; target={"balancerTag":"proxy-pool"}
...
def genn(d):
 ...
 if tls: txt+=f"server {{ listen 443 ssl; listen [::]:443 ssl; http2 on; {common} ssl_certificate /etc/letsencrypt/live/{dom}/fullchain.pem; ssl_certificate_key /etc/letsencrypt/live/{dom}/privkey.pem; ssl_protocols TLSv1.2 TLSv1.3; }}\n"
...
def install(d): print("Installed",install_xray()); service(); apply(d); Path("/etc/cron.d/xray-mobile").write_text("17 3 * * * root /usr/local/sbin/xray-mobile --purge-expired\n")
def showcfg(d,u,p):
...
def pmenu(d):
 while 1:
  dash(d,"SOCKS5 / HTTP"); c=menu("Proxy",[("1","Enable/disable"),("2","Mode full/domain/off"),("3","Add"),("4","List"),("5","Select"),("6","Delete"),("7","Test"),("8","Domains"),("9","Rotate"),("a","Show secrets"),("0","Back")])
  if c=="0":return
  try:
   ...
   elif c=="9": d["proxy"]["rotate"]=not d["proxy"]["rotate"];save(d);apply(d)
 ...
 while 1:
  ...
  try:
   if c=="1":install(d)
    ...
    if input("Taip UNINSTALL: ")=="UNINSTALL": sh(["systemctl","disable","--now","xray"],False);Path("/etc/systemd/system/xray.service").unlink(missing_ok=True);NL.unlink(missing_ok=True);NC.unlink(missing_ok=True);XB.unlink(missing_ok=True)
   elif c=="3":vmenu(d)
   elif c=="4":pmenu(d)
   elif c=="5":sysmenu(d)
   elif c=="0":return
