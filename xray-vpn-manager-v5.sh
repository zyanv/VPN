...
load_balance_menu(){
  while true; do
    clear_screen; header "Load Balancing"
    echo "Status    : $(json_get '.proxy.loadBalance.enabled')"
    echo "Strategy  : $(json_get '.proxy.loadBalance.strategy')"
    echo "Probe URL : $(json_get '.proxy.loadBalance.probeUrl')"
    echo "Interval  : $(json_get '.proxy.loadBalance.probeInterval')"
    echo "Proxy aktif: $(jq '[.[]|select(.enabled==true)]|length' "$PROXIES")"
    echo
...
proxy_menu(){
  while true; do clear_screen; header "SOCKS5 / HTTP Outbound"; two_col "1) Enable/disable sistem" "7) Check proxy" "2) Mode full/domain" "8) Domain bypass list" "3) Load balancing" "9) Show config" "4) Add proxy" "10) Set proxy weight" "5) Enable proxy item" "11) Health dashboard" "6) Delete proxy" "0) Back"; read -r -p "Pilih: " c
    case "$c" in
...
header(){
  local title="${1:-ZYANV VPN MANAGER v${VERSION}}" xray nginx proxy ip domain count dns pxy ver
  ...
  printf "%b│%b LB: %-17s Mode: %-26s %b│%b\n" "$C_BLUE" "$C_RESET" "$(json_get '.proxy.loadBalance.strategy' | xargs)" "$(json_get '.proxy.mode' | xargs)" "$C_BLUE" "$C_RESET"
